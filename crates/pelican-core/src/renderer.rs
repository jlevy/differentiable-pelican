...
pub fn make_grid<B: Backend>(height: usize, width: usize, device: &B::Device) -> Tensor<B, 3> {
    // Build y values: [0, 1/(h-1), 2/(h-1), ..., 1]
    let h = height as f32;
    let w = width as f32;

    let y_vals: Vec<f32> = (0..height)
        .map(|i| if height > 1 { i as f32 / (h - 1.0) } else { 0.5 })
        .collect();
    let x_vals: Vec<f32> = (0..width)
        .map(|i| if width > 1 { i as f32 / (w - 1.0) } else { 0.5 })
        .collect();
